**오버라이딩(Override)** 은 부모 클래스의 메서드를 자식 클래스에서 **재정의** 하는것을 의미한다. 메서드 이름, 매개변수의 개수와 타입, 반환형이 부모 클래스의 메서드와 완전히 동일해야 하며, 메서드의 내용만 변경할 수 있다. 다형성을 구현하기 위해 사용되며, 프로그램 실행 중(런타임) 동적으로 호출될 메서드를 결정한다.
